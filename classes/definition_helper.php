...
class definition_helper {
    ...
    /**
     * Get valid class from type and throw exceptions if not valid.
     * @param $type
     * @param $paramname
     * @throws coding_exception
     * @return string
     */
    private function get_valid_class_from_type($type, $paramname): string {
        $classdetected = $this->get_class_from_type($type);

        if (empty($classdetected)) {
            throw new coding_exception(
                    $this->param_error('Unknown / incompatible var type ' . $type, $paramname)
            );
        }

        if ($classdetected === $this->classname) {
            throw new coding_exception($this->param_error('Class definition infinite recursion ', $paramname));
        }
    ...
    private function _define_class_for_webservice($classorobject) {
        ...
        foreach ($public as $property) {
            ...
            if ($wstype) {
                if (substr($wstype, -2, 2) === '[]') {
                    $isarray = true;
                    $wstype = substr($wstype, 0, -2);
                }
                if (defined($wstype)) {
                    $wstype = constant($wstype);
                } else {
                    $classdetected = $this->get_valid_class_from_type($wstype, $name);
                }
            }
            ...
            if ($hasvardoc !== 1 && (empty($wstype) && empty($wsdesc))) {
                throw new coding_exception('Property '.$name.' missing @var, @wstype or @wsparam doc in class '.$this->classname);
            }
            ...
            if (!empty($wstype)) {
                ...
                if (isset($aliases[$type])) {
                    ...
                    if ($type === 'stdClass') {
                        throw new coding_exception(
                                'Property ' . $name . ' using unsupported stdClass in class ' . $this->classname
                        );
                    }
                    $classdetected = $this->get_valid_class_from_type($type, $name);
                }
            }
            ...
            if ($classdetected) {
                ...
                if ($isarray) {
                    $this->definition[$name] = new external_multiple_structure(
                            new external_value($type, $description, $wsreq, null, $wsallownull)
                    );
                } else {
     ...
     * @param string|object $classorobject
     */
    public static function define_for_webservice($classorobject) {
